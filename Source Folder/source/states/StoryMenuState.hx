...
class StoryMenuState extends MusicBeatState
{
	...
	var difficultyXY:Array<Float> = [100,400];
	...
	override function create()
	{
		...
		leftArrow = new FlxSprite(difficultyXY[0], difficultyXY[1]);
		...
		rightArrow = new FlxSprite(difficultyXY[0], difficultyXY[1]);
		...
		txtTracklist = new FlxText(FlxG.width * 0.05, tracksSprite.y + 60, 0, "TRACKS:", 32);
		txtTracklist.alignment = CENTER;
		txtTracklist.setFormat(Paths.font("Schluber.ttf"), 32);
	...
	function changeDifficulty(change:Int = 0):Void
	{
		...
		if(sprDifficulty.graphic != newImage)
		{
			...
			leftArrow.x = sprDifficulty.x+(sprDifficulty.width/2)-(leftArrow.width/2);
			leftArrow.y = sprDifficulty.y+sprDifficulty.height;

			rightArrow.x = sprDifficulty.x+(sprDifficulty.width/2)-(rightArrow.width/2);
			rightArrow.y = sprDifficulty.y-rightArrow.height;
