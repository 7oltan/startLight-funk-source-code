...
class PlayState extends MusicBeatState
{
	...
	public static var galamix:Bool = false;
	...
	var player1Bar:FlxSprite;
	var player2Bar:FlxSprite;
	...
	override public function create()
	{
		...
		player1Bar = new FlxSprite(healthBar.x,healthBar.y-100);
		player1Bar.antialiasing = true;
		player1Bar.cameras = [camHUD];
		player1Bar.flipX = true;
		player1Bar.alpha = ClientPrefs.data.healthBarAlpha;
		player1Bar.visible = !ClientPrefs.data.hideHud;
		add(player1Bar);

		player2Bar = new FlxSprite(healthBar.x,healthBar.y-100);
		player2Bar.antialiasing = true;
		player2Bar.cameras = [camHUD];
		player2Bar.alpha = ClientPrefs.data.healthBarAlpha;
		player2Bar.visible = !ClientPrefs.data.hideHud;
		add(player2Bar);
	...
	public function reloadHealthBarColors() {
		...
		player1Bar.loadGraphic(Paths.image('bars/'+boyfriend.healthBar));
		if(boyfriend.healthBar == "default")
			player1Bar.color = FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
		else
			player1Bar.color = FlxColor.WHITE;
		player1Bar.updateHitbox();
		player1Bar.screenCenter(X);

		
		player2Bar.loadGraphic(Paths.image('bars/'+dad.healthBar));
		if(dad.healthBar == "default")
			player2Bar.color = FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);
		else
			player2Bar.color = FlxColor.WHITE;
		player2Bar.updateHitbox();
		player2Bar.screenCenter(X);
	}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if(gottaHitNote)
					charN = boyfriend;
				swagNote.rgbShader.r = FlxColor.fromRGB(charN.healthColorArray[0], charN.healthColorArray[1], charN.healthColorArray[2]);
				swagNote.rgbShader.b = FlxColor.fromRGB(charN.healthColorArray[0]+21, charN.healthColorArray[1]-189, charN.healthColorArray[2]-66);
				swagNote.rgbShader.g = FlxColor.WHITE;*/
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						/*sustainNote.rgbShader.r = FlxColor.fromRGB(charN.healthColorArray[0], charN.healthColorArray[1], charN.healthColorArray[2]);
						sustainNote.rgbShader.b = FlxColor.fromRGB(charN.healthColorArray[0]+21, charN.healthColorArray[1]-189, charN.healthColorArray[2]-66);
						sustainNote.rgbShader.g = FlxColor.WHITE;*/
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			if(player == 1)
				charN = boyfriend;
			babyArrow.rgbShader.r = FlxColor.fromRGB(charN.healthColorArray[0], charN.healthColorArray[1], charN.healthColorArray[2]);
			babyArrow.rgbShader.b = FlxColor.fromRGB(charN.healthColorArray[0]+21, charN.healthColorArray[1]-189, charN.healthColorArray[2]-66);
			babyArrow.rgbShader.g = FlxColor.WHITE;*/
	...
	override public function update(elapsed:Float)
	{
		...
		if(boyfriend.animation.curAnim != null && dad.animation.curAnim != null){
			...
			if(charC.animation.curAnim.name.startsWith("singRIGHT"))
					XX += BANG;
			if(charC.animation.curAnim.name.startsWith("singUP"))
					YY -= BANG;
			if(charC.animation.curAnim.name.startsWith("singDOWN"))
					YY += BANG;
		}
		...
		var P2BR:FlxRect = new FlxRect(0,0, player2Bar.frameWidth,player2Bar.frameHeight);
		P2BR.width = player2Bar.frameWidth*((2-health)/2);
		player2Bar.clipRect = P2BR;

		var P1BR:FlxRect = new FlxRect(0,0, player1Bar.frameWidth,player1Bar.frameHeight);
		P1BR.width = player1Bar.frameWidth*(health/2);
		player1Bar.clipRect = P1BR;
		...
		if(ret != FunkinLua.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(galamix)
						MusicBeatState.switchState(new GalamixMenuState());
					else
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		var rating:FlxSprite = new FlxSprite(95,570);
		var score:Int = 350;
		...
		if((daRating.noteSplash && !note.noteSplashData.disabled) || note.splashEveryTime)
		...
		{
			var numScore:FlxSprite = new FlxSprite(rating.x+30-(43 * 3)+(43 * daLoop),rating.y-30).loadGraphic(Paths.image(uiPrefix + 'num' + Std.int(i) + uiSuffix));
